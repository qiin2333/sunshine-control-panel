...
#[cfg(target_os = "windows")]
const ELEVATED_INSTALL_ARG: &str = if IS_NR {
    "--elevated-dlssnr-install"
} else {
    "--elevated-rtx-hdr-install"
};
#[cfg(target_os = "windows")]
const ELEVATED_REMOVE_ARG: &str = if IS_NR {
    "--elevated-dlssnr-remove"
} else {
    "--elevated-rtx-hdr-remove"
};
#[cfg(target_os = "windows")]
const ELEVATED_RECOVER_ARG: &str = if IS_NR {
    "--elevated-dlssnr-recover"
} else {
    "--elevated-rtx-hdr-recover"
};
...
use super::EnhancementComponentStatus;

fn selected_backend(settings: &crate::hdr_enhanced::Settings) -> &Option<String> {
    if IS_NR {
        &settings.selected_nr_backend
    } else {
        &settings.selected_backend
    }
}

fn selected_backend_mut(settings: &mut crate::hdr_enhanced::Settings) -> &mut Option<String> {
    if IS_NR {
        &mut settings.selected_nr_backend
    } else {
        &mut settings.selected_backend
    }
}

fn nr_host_supported(catalog: &serde_json::Value) -> bool {
    catalog["adapters"][COMPONENT_ID]["foundation_dlssnr_adapter.dll"]
        .as_str()
        .is_some_and(|value| value.len() == 64 && value.bytes().all(|b| b.is_ascii_hexdigit()))
}

fn component_root() -> PathBuf {
    crate::native_components::core_component_root(COMPONENT_ID)
        .expect("enhancement backend must be registered as a Core component")
}
...
fn trusted_manifest(
    ...
    if IS_NR {
        if !nr_host_supported(catalog)
            || component_id.len() != 64
            || !component_id.bytes().all(|b| b.is_ascii_hexdigit())
        {
            return Err(
                "HDR-PKG-009: invalid DLSS NR component identity or unsupported Core".into(),
            );
        }
        // NR is imported locally and pinned by the user-selected file digest.
        // This is not a vendor authentication claim or a remote runtime allowlist.
        return Ok(ComponentManifest {
            schema: 1,
            component_id: component_id.into(),
            runtime_sha256: component_id.to_ascii_lowercase(),
        });
    }
    let trusted = catalog
        .get("components")
...
fn build_status(
    ...
    let hdr_supported = catalog
        .get("components")
        .and_then(|components| components.get(COMPONENT_ID))
        .and_then(serde_json::Value::as_object)
        .is_some_and(|versions| !versions.is_empty());
    let host_supported = if IS_NR {
        nr_host_supported(catalog)
    } else {
        hdr_supported
    };
    ...
    let enabled = selected_backend(&config.settings).as_deref() == Some(COMPONENT_ID);
    ...
    let has_state = |state: &str| {
        pipelines.is_some_and(|pipelines| {
            pipelines.iter().any(|pipeline| {
                pipeline
                    .get(if IS_NR { "nr_backend" } else { "backend" })
                    .and_then(serde_json::Value::as_str)
                    == Some(COMPONENT_ID)
                    && pipeline
                        .get(if IS_NR { "nr_state" } else { "state" })
                        .and_then(serde_json::Value::as_str)
                        == Some(state)
            })
        })
...
async fn ensure_idle() -> Result<(), String> {
    ...
    if !state.sessions.is_empty() {
        return Err(
            "HDR-SESSION-002: finish all active streams before changing enhancement components"
                .to_string(),
        );
    }
    Ok(())
...
async fn publish_installed_version(
    ...
    current.settings.backends.insert(
        ...
        crate::hdr_enhanced::BackendSettings {
            ...
            runtime_sha256: IS_NR.then(|| manifest.runtime_sha256.clone()),
        },
    );
...
pub(crate) fn try_handle_elevated_command() -> Option<i32> {
    ...
    let mut run = || -> Result<(), String> {
        ...
        let operation =
            executor.block_on(crate::hdr_enhanced::verify_maintenance(COMPONENT_ID, &id))?;
...
async fn install_runtime_with_elevation(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let operation =
            crate::hdr_enhanced::verify_maintenance(COMPONENT_ID, &operation.id).await?;
...
async fn finish_operation(operation_id: &str) -> Result<(), String> {
    #[cfg(target_os = "windows")]
    ensure_helper_finished().await?;
    crate::hdr_enhanced::finish_maintenance(COMPONENT_ID, operation_id).await
}

pub async fn component_install(runtime_path: String) -> Result<EnhancementComponentStatus, String> {
...
pub async fn component_set_enabled(enabled: bool) -> Result<EnhancementComponentStatus, String> {
    let _operation = COMPONENT_OPERATION
        .try_lock()
        .map_err(|_| "HDR-OP-001: another component operation is running".to_string())?;
    let mut current = crate::hdr_enhanced::get_config().await?;
    ...
    *selected_backend_mut(&mut current.settings) = enabled.then(|| COMPONENT_ID.to_string());
...
mod tests {
    ...
    fn runtime_identity_is_independent_of_the_adapter_build() {
        ...
        let mut catalog = serde_json::json!({
            "schema_version": 1,
            "adapters": { COMPONENT_ID: { "foundation_dlssnr_adapter.dll": "c".repeat(64) } },
            "components": { COMPONENT_ID: { hash.clone(): { RUNTIME_FILE: hash } } }
        });
    ...
    fn degraded_pipeline_is_not_hidden_by_an_active_pipeline() {
        ...
        let runtime = serde_json::json!({"pipelines":[
            {"backend":COMPONENT_ID,"state":"active","nr_backend":COMPONENT_ID,"nr_state":"active"},
            {"backend":COMPONENT_ID,"state":"degraded","nr_backend":COMPONENT_ID,"nr_state":"degraded"}
        ]});
    ...
    fn static_integrity_check_does_not_execute_imported_dlls() {
        ...
        assert!(
            crate::native_components::descriptor(COMPONENT_ID)
                .unwrap()
                .distribution
                .local_import
        );
