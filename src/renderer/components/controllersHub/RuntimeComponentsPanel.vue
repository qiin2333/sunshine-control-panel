  ...
  <section class="chub-panel chub-runtime-components">
    <div class="chub-section">
      ...
      <div v-else class="chub-cards">
        ...
        <article class="chub-card">
          <div class="chub-card-head"><strong>USB/IP Transport</strong><el-tag size="small" :type="usb.ready ? 'success' : 'warning'" effect="plain">{{ usb.ready ? t.deviceHub.available : t.deviceHub.unavailable }}</el-tag></div>
          <p class="chub-hint">{{ t.deviceHub.components.transportHint }}</p><p class="chub-status-text">{{ usb.version || t.deviceHub.unknown }}</p>
        </article>
      </div>
