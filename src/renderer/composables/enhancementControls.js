...
const messages = {
  zh: {
    ...
    cancel: '取消录制',
    gamepadHint: '支持 Windows Xbox / XInput 手柄（含串流虚拟手柄）。同时按住 LB、RB 和所选 X/Y 半秒触发，全部松开后可再次使用；按键仍会传给游戏。大屏中参与组合的单键会在松开时执行原导航。',
    reset: '恢复默认',
  ...
  en: {
    ...
    keyboard: 'Keyboard',
    gamepad: 'Gamepad',
    gamepadChecking: 'Checking background controllers…',
    ...
    gamepadHint: 'Windows Xbox / XInput controllers, including virtual streaming controllers. Hold LB, RB and the selected X/Y for half a second; release all buttons before using again. Games still receive these buttons. In desktop mode, a shortcut button used alone navigates on release.',
